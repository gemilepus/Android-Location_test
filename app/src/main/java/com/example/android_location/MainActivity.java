...
public class MainActivity extends AppCompatActivity {
    ...
    private void requestLocationPermission() {
        if (ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.ACCESS_FINE_LOCATION)) {
            Log.d("debug", "requestLocationPermission() shouldShowRequestPermissionRationale");

            Snackbar sbar = Snackbar.make(findViewById(android.R.id.content), R.string.message1, Snackbar.LENGTH_INDEFINITE);
            sbar.getView().setBackgroundColor(Color.rgb(32, 125, 98));

            sbar.setAction(R.string.sbar_button1, new View.OnClickListener() {
                @Override
            ...
            Log.d("debug", "requestLocationPermission() else");
    ...
    public void onRequestPermissionsResult(
        ...
        if (requestCode == REQUEST_PERMISSION) {
            ...
            if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                // 拒否された時の対応
                Snackbar sbar = Snackbar.make(findViewById(android.R.id.content), R.string.message2, Snackbar.LENGTH_SHORT);
    ...
    private void updateLocationUI() {
        if (mCurrentLocation != null) {
            strBuf.append((String.format(Locale.ENGLISH, "%s: %f,  ", "Latitude(緯度)", mCurrentLocation.getLatitude())));
            strBuf.append((String.format(Locale.ENGLISH, "%s: %f\n", "Longitude(經度)",mCurrentLocation.getLongitude())));
            strBuf.append((String.format(Locale.ENGLISH, "%s: %s\n", "Time", mLastUpdateTime)));
